...
class TodoList extends Component {
  ...
  // 把大量内容挪出来单独创建函数保存使代码不会太冗长庞大，便于维护
  getTodoItem(){
    // 如果要做循环，那么每个循环都要给一个K值，每个Todolist都要由key值，这个后面细说
    //如果不要div元素，这个key值可以写在TodoItem里 
	...
	handleInputChange(e) {
		...
		//this.setState({//让input框数据变成e.target.value  inputValue: e.target.value})
	...
	handleBtnClick() {
		// steState可以接收一个叫prevState的参数，这个参数意思是代表之前的状态
